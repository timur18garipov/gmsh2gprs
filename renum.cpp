...
void renum::convertEmil(vector<int>& rcm, vector<vector<double>>& coords)
{

	// rcm - mapping between additional nodes and initial nodes
	// coords - coordinates of all the nodes in actual numbering

	int nNodes = rcm.size();
	for (int k = 0; k < nNodes; ++k)
	{
		vector<double> icoord = coords[k];
