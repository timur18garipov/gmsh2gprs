...
void CalcTranses::createKarimiData()
{
  ...
  vIbHasGhostIb.assign(NbFracs+NbPolyhedra,false);  
  for(int ic = 0; ic < pSim->nCells; ++ic)    
  {
    if(pSim->vCellsHasGhost[ic])
      vIbHasGhostIb[ic+NbFracs] = true;
  }
  
  nGhostIb = 0;
  vIbToGhostIb.assign(NbFracs+NbPolyhedra,-1);
  for(int ib = 0; ib < NbFracs+NbPolyhedra; ++ib)    
  {
    if(vIbHasGhostIb[ib])
    {
      vIbToGhostIb[ib] = NbFracs+NbPolyhedra + nGhostIb;
      nGhostIb++;
    }
  }  
}
...
//////////////////////////////////////////////////////////////////////////////////
/////                                                                        /////
///// OUTPUT FOR GEOMECHANIC -- THE LIST OF TRANSMISSIBILITIES TO BE UPDATED /////
/////                                                                        /////
//////////////////////////////////////////////////////////////////////////////////
    /*
    string outfile = "small.uptrans";    
    poutfile=fopen( outfile.c_str(),"a");
    ...
    for (i=0;i<NbConnections;i++)
    {
        if (ConType[i]==1 || ConType[i]==2)	// M-M, M-F ////////////////////////////
        {
            if (ConType[i]==2)
            {
                fprintf(poutfile,"%d\t%d\t%e\t%d\t%e\t%d\t%d\t%e\t%d\t%e\n",
                        k,
                        ConCV[i][0],ZVolumeFactor[CVZone[ConCV[i][0]]]*ConTr[i][0],
                        ConCV[i][1],ZVolumeFactor[CVZone[ConCV[i][1]]]*ConTr[i][1],
                        2,
                        ConCV[i][0],ZVolumeFactor[CVZone[ConCV[i][0]]]*ConTr[i][0],
                        ConCV[i][1],ZVolumeFactor[CVZone[ConCV[i][1]]]*ConTr[i][1]);
            }
        ...
        if (ConType[i]==3)	// F-F /////////////////////////////////////////////////
        {
            ...
            for (j=0;j<ConN[i]-1;j++)
                for (n=j+1;n<ConN[i];n++)
                {
                    fprintf(poutfile,"%d\t%d\t%e\t%d\t%e\t",
                            k,
                            ConCV[i][j],ZVolumeFactor[CVZone[ConCV[i][j]]]*ConTr[i][j],
                            ConCV[i][n],ZVolumeFactor[CVZone[ConCV[i][n]]]*ConTr[i][n]);
                    fprintf(poutfile,"%d\t",ConN[i]);
                    for (m=0;m<ConN[i];m++)
                        fprintf(poutfile,"%d\t%e\t",ConCV[i][m],ZVolumeFactor[CVZone[ConCV[i][m]]]*ConTr[i][m]);
                    fprintf(poutfile,"\n");
                    k++;
                }
        }
    ...
    */
    ...
    string outstring =   "fl_tran_u.txt";
